...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            ...
            ProgressView("Task Progress", value: progress, total: 100)
                .progressViewStyle(MyCircleProgressViewStyle())
...
struct MyCircleProgressView: Shape {
...
struct MyCircleProgressViewStyle: ProgressViewStyle {
    func makeBody(configuration: Configuration) -> some View {
        let degrees = configuration.fractionCompleted! * 360
        let percent = Int(configuration.fractionCompleted! * 100)
        return VStack {
            MyCircleProgressView(startAngle: .degrees(1), endAngle: .degrees(degrees))
